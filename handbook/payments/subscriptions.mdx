...
| Subscription updated (plan changed, paused, past due) | Plan, amount and status follow. Stripe's active or trialing read Active; past due or unpaid read Past Due; paused reads Paused | The same fields follow |
...
- **One writer for the Zoho Subscription Payment.** Only the engine's sync push makes the Zoho record, keyed on our payment. The Stripe step writes our payment row and nothing in Zoho, so one paid invoice is one Zoho record. A Subscription Payment in Zoho with no Status or no Amount was not made this way; delete it in Zoho.

## Plan visits

A work order counts as a plan visit when its **Subscription** lookup in Zoho names the customer's subscription and its lines are the plan's covered maintenance products. Those lines are priced at nothing, and a job with no other lines is marked Non-Billable and never invoiced. The billing decision itself is on [How booking works](/handbook/booking/how-booking-works).
...
The Subscription link on a work order belongs to Zoho. Set it there, as the dispatch calendar does. A link written only on our side is cleared by Zoho's next echo, so the job loses its plan status.

## A payment typed into Zoho is refused

Stripe is the only way a subscription payment is made. A Subscription Payment typed into Zoho by hand is refused: nothing is written here, and the refusal says the Zoho record can never sync. Delete it in Zoho. A value typed in Zoho over a field this side owns on a subscription or a subscription payment is refused the same way: a human-review flag names the field and the engine pushes its own value back. See [Resolve a human-review flag](/handbook/running/resolve-a-human-review-flag).

## How to test it

The account holds one live Stripe key and no test-mode key, so a test subscription cannot be billed without charging a real card. Developers use the replay harness in the [backend repo](/reference/repos-and-links) instead, which plays Stripe's events against the production endpoint; it only accepts a customer whose name starts with "Probe", and its cleanup takes back everything it made.
