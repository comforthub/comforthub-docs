...
| Invoice paid, $19.99 | A subscription payment for the invoice; the roll-up reads Months Paid 1, Paid This Term 19.99 | Subscription Payment, Paid, with the payment date (since 5 September made by the sync push alone, named "Payment in_…" and carrying the amount, the period and the Stripe invoice id as well; see below); the Subscription's Months Paid, Paid This Term and Last Payment Date | payment 1.8 s here, 3.0 s in Zoho; the roll-up 4.7 s |
...
## One writer for the subscription payment

Until 5 September 2026 one paid invoice could become two Subscription Payments in Zoho (<Linear issue="COM-208" />). The Stripe step wrote the Zoho record itself, keyed on the Stripe invoice id, while the payment row's own sync push created it again, keyed on our payment id. Neither knew about the other, so whichever ran second made a copy: one with a Status and a payment date but no amount, one with an amount and a period but no Status. Twice on 4 September the copy's echo also made a second payment row here, and Months Paid read 2 for one payment.

Since 5 September the Stripe step writes only our payment row. The sync push alone makes the Zoho Subscription Payment, named "Payment in_…" after the Stripe invoice, with Amount, Period Start, Period End, Payment Date, Status Paid and the Stripe invoice id all on the one record. Months Paid on the Subscription counts our payment rows, so one payment reads 1. The Status is always Paid because only a paid invoice makes a payment; a failed one is recorded and makes no row.

A Zoho Subscription Payment with no Status or no Amount is a leftover from before that day. Delete it in Zoho; nothing here refers to it.
