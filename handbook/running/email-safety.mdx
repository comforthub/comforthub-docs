...
Every email the engine sends passes one gate before it leaves. The gate has a short list of addresses that may receive mail. Anything else is not sent to the customer: it is sent to the operator instead, with the intended address written into the subject. This page is the one home for that rule; [Resend](/systems/resend) and [Emails we send](/reference/emails-we-send) point here.

## The three outcomes

| The address is | What happens | How it looks |
|---|---|---|
| On the allowed list: the operator's own address | delivered as written | normal subject |
