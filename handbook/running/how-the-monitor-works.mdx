...
| Uncertain or overdue payments, cancelled work still collectible, a payroll calendar that ran out, a timesheet with no period, a card number in stored text, a critical action late or dead, a timer changed by hand, a check that did not complete | Invoice totals that disagree, a lead with no customer behind it, a plain human-review flag, a property without imagery, a routine action late or dead, housekeeping that did not finish, a nightly GitHub job that did not report, a timer nobody deployed |
...
- **A check fails or times out.** It closes nothing, and the self-check that watches completion opens an urgent condition naming the check.
