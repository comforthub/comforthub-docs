...
The evaluator books first and runs second. A booking is a row that says "this check was due at this minute", so if the evaluator dies half way, the missing result is still visible to the self-check that looks for it. Every check has a time budget, and one that runs out of it saves its place and carries on at the next wake rather than starting over. Quick checks run inside the evaluator's own minute; slow ones are booked the same way and handed to the maintenance worker, which takes them off its queue.
...
- **Self-checks** watch the engine's own clockwork: every timer still matches what was deployed, every scheduled database job left evidence that it ran, every check completed on time, no queue holds a message older than fifteen minutes, and the nightly jobs that run on GitHub reported back.
...
- **Is the monitor keeping up?** Yes only when the monitor is switched on, the evaluator's latest run finished cleanly and recently, no finished result is still waiting to be turned into conditions, no notification has waited longer than its allowance, and the stand-in technician is still the switched-off, registered technician it should be. The limits are on [Sentry](/systems/sentry).
