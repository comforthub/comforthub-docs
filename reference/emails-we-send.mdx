...
Every email leaves through one sender that keeps a ledger and checks the address first; only allowed addresses are delivered to as written, and everything else goes to the operator instead. The rule is on [Email safety](/handbook/running/email-safety). The engine sends no operator alert email: a problem the monitor finds becomes a condition on [Sentry](/systems/sentry), and Sentry does the mailing.
...
| Invoice | Invoice INV… from Comfort Hub | customer | the invoice gets its QuickBooks counterpart, and is not void, paid or already sent | the invoice PDF; on a completed maintenance or commissioning job, one certificate PDF per certifiable visit; a pay link |
...
| Receipt | Payment received for INV… | customer | a payment on the invoice is recorded, whether or not it clears the balance; the body shows the amount paid and the balance left | the receipt PDF and the current invoice PDF |
...
A demo invoice, made for testing the pay page, gets the same invoice and receipt emails with "Demo" in the receipt's subject and body.
