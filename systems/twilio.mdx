...
An agent with no department of their own belongs to every active department. The routing script writes one queue per active department, keeps the main queue as the catch-all, and writes the workflow's per-department rule (the department's queue for its ring length, then the main queue). It runs dry by default and reports what it would change; a second write run must find nothing to change.
